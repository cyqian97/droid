...
### Proto Definition (`proto/franka_control.proto`)
...
### Python Client (`FrankaDirectClient`)
...
## Server: Joint Torque Control (`franka_server.cpp`)
...
## Server: Cartesian Velocity Control (`franka_server_cartesian.cpp`)

### Thread Architecture

Same three-thread pattern as the joint server, but the RT callback returns
...
### `config/controller.yaml` (Joint Server)
...
### `config/controller_cartesian.yaml` (Cartesian Server)
...
The diagram below shows which files participate in the end-to-end
communication pipeline, from user-facing Python scripts down to the robot
hardware.  Files are grouped by where they run (laptop vs NUC Docker
container vs robot).
...
3. **`franka_control.proto`** defines the `EETarget` message and the
   `SetEETarget` RPC — both C++ server and Python stubs are generated from
   this single file.
4. **`franka_server_cartesian.cpp`** receives the RPC on a gRPC thread,
   writes the new goal pose into `SharedState` (mutex), and returns
   `CommandResult` immediately.
5. The **1 kHz RT callback** (same `.cpp` file, inside `robot.control()`)
...
### `franka_direct/` — Core Module

| File | Role |
|------|------|
| `proto/franka_control.proto` | **Interface contract.** Defines the `FrankaControl` gRPC service (6 RPCs) and all message types (`JointTarget`, `EETarget`, `GripperTarget`, `JointResetTarget`, `RobotState`, etc.).  Both the C++ servers and the Python stubs are generated from this single file.  Edit this when adding new RPCs or fields. |
| `src/franka_server.cpp` | **Joint torque server.**  Connects to the robot via libfranka, runs a 1 kHz `robot.control()` callback that returns `franka::Torques`.  Receives `SetJointTarget` RPCs on gRPC threads, interpolates toward the goal at 1 kHz, and applies a PD torque controller + Coriolis feedforward.  Also handles gripper commands on a dedicated thread. |
| `src/franka_server_cartesian.cpp` | **Cartesian velocity server.**  Same architecture as the joint server, but the RT callback returns `franka::CartesianVelocities`.  Receives `SetEETarget` RPCs, computes a PD velocity command from the pose error (position + orientation), and applies a cosine velocity ramp for smooth start/stop.  Also implements `ResetToJoints` (cosine-interpolated joint-space motion). |
| `config/controller.yaml` | Tunable parameters for the joint server: PD gains (`kp`, `kd`), torque limits, interpolation step size, LPF cutoff, gripper force. |
| `config/controller_cartesian.yaml` | Tunable parameters for the Cartesian server: per-axis PD gains, velocity clamps, ramp duration, optional `init_q` startup pose, gripper force. |
...
| `build.sh` | Shell script to run CMake + make inside the Docker container.  Activates the conda environment and configures `ldconfig` so libfranka's `.so` is discoverable.  Produces `build/franka_server` and `build/franka_server_cartesian`. |
...
### `franka_direct/python/` — Client & Stubs

| File | Role |
|------|------|
| `franka_direct_client.py` | **Python gRPC client.**  A thin class (`FrankaDirectClient`) that wraps every RPC in a method returning native Python types (`list`, `dict`, `bool`).  Works with either server variant.  Imported by all demo scripts. |
| `generate_stubs.sh` | Runs `grpc_tools.protoc` on `franka_control.proto` to regenerate the two `_pb2` files below.  Run this on the laptop after any `.proto` change. |
| `franka_control_pb2.py` | **(Auto-generated)** Protobuf message classes — the Python serialization/deserialization layer.  Do not edit by hand. |
| `franka_control_pb2_grpc.py` | **(Auto-generated)** gRPC stub classes (`FrankaControlStub`) used by the client to make RPCs.  Do not edit by hand. |

### `scripts/` — Demo Applications (repository root)
